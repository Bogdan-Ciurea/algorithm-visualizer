...
// NOTE: We check for GUI_STATE_DISABLED to avoid messing custom global state
// setups
void GuiEnable(void) {
...
// NOTE: We check for GUI_STATE_NORMAL to avoid messing custom global state
// setups
...
#else
  clicked =
      GuiButton(closeButtonRec, GuiIconText(RAYGUI_ICON_CROSS_SMALL, NULL));
#endif
...
#define RAYGUI_LINE_TEXT_PADDING 8
  ...
  if (text == NULL)
    ...
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height / 2,
                                   RAYGUI_LINE_TEXT_PADDING - 2, 1},
        0, BLANK, color);
    GuiLabel(textBounds, text);
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){
            bounds.x + RAYGUI_LINE_TEXT_PADDING + textBounds.width + 4,
            bounds.y + bounds.height / 2,
            bounds.width - textBounds.width - RAYGUI_LINE_TEXT_PADDING - 4, 1},
        0, BLANK, color);
  }
  //--------------------------------------------------------------------
}
...
// Button control with color, returns true when clicked
bool GuiButtonWithColor(Rectangle bounds, const char *text, const Color color) {
  GuiControlState state = guiState;
  bool pressed = false;
  ...
  GuiDrawRectangle(bounds, GuiGetStyle(BUTTON, BORDER_WIDTH),
                   Fade(color, guiAlpha), Fade(color, guiAlpha));
  GuiDrawText(
      text, GetTextBounds(BUTTON, bounds), GuiGetStyle(BUTTON, TEXT_ALIGNMENT),
      Fade(GetColor(GuiGetStyle(BUTTON, TEXT + (state * 3))), guiAlpha));
  //------------------------------------------------------------------
...
bool GuiDropdownBox(Rectangle bounds, const char *text, int *active,
  ...
  // Update control
  //--------------------------------------------------------------------
  if ((state != GUI_STATE_DISABLED) && (editMode || !guiLocked) &&
      (itemCount > 1)) {
    Vector2 mousePoint = GetMousePosition();
  ...
  GuiDrawText(
      ...
      Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state * 3))),
           guiAlpha));  // RAYGUI_ICON_ARROW_DOWN_FILL
#endif
  //--------------------------------------------------------------------
...
bool GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue,
  ...
  if (GuiButton(leftButtonBound,
                GuiIconText(RAYGUI_ICON_ARROW_LEFT_FILL, NULL)))
    tempValue--;
  if (GuiButton(rightButtonBound,
                GuiIconText(RAYGUI_ICON_ARROW_RIGHT_FILL, NULL)))
    tempValue++;
#endif
  ...
  if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE)) {
    ...
    GuiDrawText(
        ...
        Fade(GetColor(GuiGetStyle(SCROLLBAR, TEXT + state * 3)),
             guiAlpha));  // RAYGUI_ICON_ARROW_UP_FILL /
...
#define RAYGUI_MESSAGEBOX_BUTTON_PADDING 12
  ...
  textBounds.y =
      ...
      (bounds.height - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT -
       RAYGUI_MESSAGEBOX_BUTTON_HEIGHT - RAYGUI_MESSAGEBOX_BUTTON_PADDING) /
          2 -
      textSize.y / 2;
...
#define RAYGUI_TEXTINPUTBOX_MAX_TEXT_LENGTH 256
  ...
  if (GuiTextBox(textBoxBounds, text, RAYGUI_TEXTINPUTBOX_MAX_TEXT_LENGTH,
                 textEditMode))
...
#define RAYGUI_ICON_TEXT_PADDING 4
  ...
  if ((text != NULL) && (text[0] != '\0')) {
    ...
    if (iconId >= 0) {
      ...
      // character: '\0'
      if ((text != NULL) && (text[0] != '\0'))
        textWidth += RAYGUI_ICON_TEXT_PADDING;
    }
    ...
    if (iconId >= 0) {
      ...
      GuiDrawIcon(iconId, (int)position.x,
                  (int)(bounds.y + bounds.height / 2 - RAYGUI_ICON_SIZE / 2 +
                        TEXT_VALIGN_PIXEL_OFFSET(bounds.height)),
                  1, tint);
